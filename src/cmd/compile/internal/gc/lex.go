...
l0:
	...
	/*
		L.J: Add a new lexical token "\", which creates a new OP that requires an ending, that
		makes all operations between this and the next backslash a RMW instructions.

		We want "\m,n" as the end-goal for what concurrent maps have the following structions as atomic, 
		where 'm' and 'n' are concurrent maps.

		Hence...

		\m,n: 
			m[1] += m[2] + n[1] // RMW of both m and n
			n[0] += 2 // RMW of n
			o[2] += 3 // Not a RMW of o
		\m,n

		This should be generated by the compiler as...
